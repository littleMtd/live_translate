...
def _build_qwen_optimized_prompt() -> str:
    ...
    base = (
        ...
        "   · 若遇到 3 字以上的詞彙無法在上下文中理解，或非標準韓文 → **使用 [UNK:原詞] 標記**\n"
        "   · 例如：'비맥스로'/'제트로' → [UNK:비맥스로] [UNK:제트로]（STT 捏造，勿補充意義）\n"
        "   · 例如：'메가리' → [UNK:메가리]（不認識的詞，可能是 STT 音譯）\n"
        "   · 例如：'띠부시레' → [UNK:띠부시레]（無法辨識，勿音譯成'蒂布希雷'）\n"
        "   · 例如：'이지깨' → [UNK:이지깨]（孤立的 3 字詞，無上下文）\n"
        "   · 例如：夾雜孤立漢字或異體字片段（如'手撫는다고'）→ 視為 STT 亂碼，改用上下文判斷，不要逐字硬翻\n"
        "   · 複合例：'풀리지 않는 피로의 비맥스로...' → 只翻譯有意義的部分，未知詞標記，完全無邏輯的返回空\n"
        ...
        "      - 勿推測'體重變化原因''時間安排'\n"
        "      - 勿補充'價格'、'產品功效'、'廣告文案'\n"
        "      - 遇到混亂 STT→返回空字串（不要試圖理解）\n"
        "8. **人名處理**\n"
        "   · 純韓文人名（이제, 리제, 윤진）→直接翻譯中文音譯（이제→怡潔，리제→莉潔）\n"
        "   · 韓文+英文混合（'리틀 이재'、'라이팅'）→優先選韓文部分並用中文音譯\n"
        "   · 純英文外來詞（'Little'、'Jae'）→不确定指称对象時，直接音譯成中文，避免保留英文\n"
